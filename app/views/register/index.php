	<div class="container mx-auto border mt-5 rounded-3 p-3" style="max-width: 350px;">
		...
		<form action="<?=Constant::DIRNAME?>register/daftar" method="post">
			...
			<div class="form-group mt-4 position-relative">
				<input type="text" id="username" name="username" class="form-control control-form px-2" style="padding: 10px 0px;" required minlength="4">
			...
			<div class="form-group mt-4 position-relative">
				<input type="text" id="noTelp" name="noTelp" class="form-control control-form px-2" style="padding: 10px 0px;" required>
				<label for="noTelp" class="position-absolute" style="bottom: 10px; left: 10px; font-size: 18px;">No. Telp</label>
				<i class="fa fa-phone position-absolute fs-4" style="right: 12px; bottom: 12px;"></i>
				<p class="tillana text-danger" style="font-size: 12px; position: absolute; bottom: -35px; right: 0;"></p>
			</div>

			<div class="form-group mt-4 position-relative">
				<input type="password" id="password" name="password" class="form-control control-form px-2" style="padding: 10px 0px;" required minlength="4">
			...
			<a href="<?=Constant::DIRNAME?>login" class="btn btn-primary w-100">Login</a>
	...
	<script>
		...
		alert.forEach((el) => {
			let element = el.previousElementSibling.previousElementSibling.previousElementSibling;
			element.addEventListener('keyup', function() {
				if (this.validity.valid == false) {
					if (this.value.length == 0) {
						el.innerHTML = '';
					} else {
						el.innerHTML = 'Panjang karakter harus 4';
					}
				} else {
					el.innerHTML = '';
				}
